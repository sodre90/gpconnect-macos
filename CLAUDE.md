...
`SSLContextAdapter` in `gp_saml_gui.py` exists to work around GlobalProtect gateways running outdated TLS stacks — it always enables legacy renegotiation, disables certificate verification only under `--no-verify` (`args.verify`), and additionally relaxes to weak ciphers (3DES/RC4, `SECLEVEL=1`) when `--allow-insecure-crypto` (`args.insecure`) is set. Certificate verification is **on by default**; it used to be unconditionally off, which silently exposed the prelogin request to MITM even against gateways with perfectly valid certificates. (There used to be a second, separate `TLSAdapter` class with the weak-cipher logic that was never actually mounted on the session — it's been merged into `SSLContextAdapter` and removed.) `--allow-insecure-crypto` is also forwarded to the `openconnect` command line, since `openconnect` makes its own separate TLS connection to the gateway that this adapter has no effect on.
